...
public:
    ...
    static shared_cairo_surface_t load(const std::string& filename, std::shared_ptr<egt::Rect>& rect)
    {
        std::ifstream i(filename, std::ios_base::binary);
        if (!i)
            return nullptr;
        ...
        if (!i.read(reinterpret_cast<char*>(&width), sizeof(width)) ||
            ...
            !i.read(reinterpret_cast<char*>(&y), sizeof(y)) ||
            !i.read(reinterpret_cast<char*>(&reserved), sizeof(reserved)) ||
            !i.read(reinterpret_cast<char*>(&reserved), sizeof(reserved)))
            return nullptr;
    ...
    static shared_cairo_surface_t load(const unsigned char* buf, size_t len, std::shared_ptr<egt::Rect>& rect)
    {
        const auto buf_end = buf + len;
        alignas(4) uint32_t magic = 0;
        alignas(4) uint32_t width = 0;
        ...
        // std::cout << "eraw buf: " << std::hex << static_cast<const void *>(buf) 
        //         << " ,buf_end: " << std::hex << static_cast<const void *>(buf_end) << std::endl;
    ...
    static void save(const std::string& path, unsigned char* data, int32_t x, int32_t y, uint32_t width, uint32_t height)
    {
        std::ofstream o(path, std::ios_base::binary);
        const auto magic = egt_magic();
        o.write(reinterpret_cast<const char*>(&magic), sizeof(magic));
